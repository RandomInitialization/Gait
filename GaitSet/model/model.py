...
class Model:
    def __init__(self,hidden_dim,channel,lr,hard_or_full_trip,margin,num_workers,batch_size,
                 restore_iter,total_iter,save_name,train_pid_num,frame_num,model_name,train_source,
                 test_source,img_size=64):
    ...
    def save(self):
        ...
        torch.save(self.encoder.state_dict(),
                   osp.join('checkpoint', self.model_name,
                            'Multi-Scale_{}-{:0>5}-encoder.ptm'.format(
                                self.save_name, self.restore_iter)))
        torch.save(self.optimizer.state_dict(),
                   osp.join('checkpoint', self.model_name,
                            'Multi-Scale_{}-{:0>5}-optimizer.ptm'.format(
                                self.save_name, self.restore_iter)))

    def load(self, restore_iter):
        self.encoder.load_state_dict(torch.load(osp.join(
            'checkpoint', self.model_name,
            'Multi-Scale_{}-{:0>5}-encoder.ptm'.format(self.save_name, restore_iter))))
        self.optimizer.load_state_dict(torch.load(osp.join(
            'checkpoint', self.model_name,
            'Multi-Scale_{}-{:0>5}-optimizer.ptm'.format(self.save_name, restore_iter))))
